test_that("tracker errors when neither subjid nor infile supplied", {
...
test_that("tracker derives subject id from infile and emits info when verbose", {
  td <- local_tempdir()
...
test_that("tracker builds correct command with explicit subjid, outdir and opts (no execution)", {
  td <- local_tempdir()
...
test_that("high-level wrappers (trac_all, trac_prep, trac_bedpost, trac_path) forward expected opts", {
  td <- local_tempdir()
