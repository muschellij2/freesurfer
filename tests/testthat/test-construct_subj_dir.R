describe("construct_subj_dir", {
  it("creates correct directory structure and copies files", {
  ...
  it("generates temporary ID if subj is NULL", {
    subj_root_dir <- withr::local_tempdir()
  ...
  it("creates partial folders when some parameters are NULL", {
    subj_root_dir <- withr::local_tempdir()
    mri_file <- withr::local_tempfile(fileext = ".mgz", tmpdir = subj_root_dir)
    stats_file <- withr::local_tempfile(
      fileext = ".stats",
      tmpdir = subj_root_dir
    )
