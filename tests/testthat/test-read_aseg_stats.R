describe("read_aseg_stats", {
  ...
  mock_aseg_stats_content <- function(
    ...
    if (extra_header_lines > 0) {
      content <- c(
        content,
        paste0("# extra_header_line_", 1:extra_header_lines)
      )
    }
  ...
  it("reads a valid aseg.stats file correctly (default lowercase)", {
    ...
    it("reads actual FreeSurfer aseg.stats file", {
  ...
  it("handles file starting with lines before # subjectname", {
    mock_content <- mock_aseg_stats_content(
      num_measures = 1,
      num_structures = 1,
  ...
  it("throws error for non-existent file", {
    ...
    non_existent_file <- file.path(temp_dir, "non_existent.stats")
    expect_error(
  ...
  it("handles empty file (no content)", {
    withr::with_tempfile(
      "mock_empty_file",
      fileext = ".stats",
      {
